        ...
        for part in tqdm(range(num_splits), desc="创建分割文件"):
            ...
            with h5py.File(out_file, "w") as f_out:
                ...
                for name in part_datasets:
                    ...
                    # 读取原始数据集
                    original_data = f_in[f"data/{name}"][:]
                    
                    # 检查维度并处理
                    if len(original_data.shape) == 2:
                        # 二维数据 (3, 3215) -> 扩充为 (1, 3, 3215)
                        expanded_data = original_data[None, ...]  # 在最前面添加一个维度
                        print(f"扩充维度: {name} {original_data.shape} -> {expanded_data.shape}")
                        
                        # 创建新的数据集
                        grp_out.create_dataset(name, data=expanded_data, dtype=original_data.dtype)
                    else:
                        # 其他维度数据，直接复制
                        f_in.copy(f"data/{name}", grp_out)
                        print(f"直接复制: {name} {original_data.shape}")
                    
                    bucket_time = time.time() - bucket_start_time
                    bucket_times.append(bucket_time)
                    print(f"处理 {name} 完成 (耗时 {bucket_time:.3f}s)")
